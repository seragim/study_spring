...
public class PageVO {
	...
	public void setTotalList(int totalList) {
		this.totalList = totalList;	//DB로부터 조회된 총 글건수
		totalPage = totalList / pageList;	//총 페이지 수 = 총건수 / 페이지당 글건수
		if( totalList % pageList > 0 ) ++totalPage;
		
		totalBlock = totalPage / blockPage;	//총 블럭 수 = 총페이지수 / 블럭당 페이지수
		if( totalPage % blockPage > 0 ) ++totalBlock;
		
		// 현재 페이지에 따라 보여질 글의 시작/끝목록 번호가 결정된다.
		//각 페이지의 끝 목록번호 : 총목록수 - (페이지 번호 -1) * 페이지 당 보여질 목록수
		endList = totalList - ( curPage - 1 ) * pageList;
		// 각 페이지의 시작 목록번호 :  끝 목록번호 - (페이지당 보여질 목록수-1)
		beginList = endList - ( pageList - 1 );
		
		// 각 블럭에 보여질 페이지번호는 현재 블럭에 따라 결정된다
